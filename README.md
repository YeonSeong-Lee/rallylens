...
### 코트 탑뷰 궤적 — Court Trajectory

<img src="outputs/demo/viz_court.gif" width="300" alt="코트 궤적 GIF">

*선수·셔틀콕 궤적 애니메이션 + 히트맵 배경 — Player & shuttle trails over a heatmap backdrop.*
...
- **시각화** — 오버레이 MP4 (탑뷰 PIP 포함) + 코트 탑뷰 궤적 GIF (히트맵 배경)
...
| `--skip-viz` | off | 코트 GIF 자동 렌더 건너뛰기 |
...
3. **Markdown 은 결정론적 템플릿**: `render_report_markdown` 은 LLM 을 두 번 호출하지 않습니다. 수치는 `MatchMetrics` 에서 직접 인용하고, 코트 GIF 는 `os.path.relpath` 로 `report.md` 에 상대 경로로 임베드됩니다.
